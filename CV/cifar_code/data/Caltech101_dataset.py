# -*- coding: utf-8 -*-
"""
Created on Fri Aug 23 21:59:55 2019
...
from torchvision.datasets.utils import download_and_extract_archive, makedir_exist_ok, verify_str_arg
...
class Caltech101(VisionDataset):
    ...
    def __init__(self, root , train = True,target_type="category", transform=None,
        ...
        temp = list(zip(self.index, self.y))  
        np.random.seed(0)
        np.random.shuffle(temp)
        self.index,self.y = zip(*temp)
        
        self.train_size = int(0.8*len(self.index))
        self.test_size = len(self.index) - int(0.8*len(self.index))
